...
;
  calculatePixelForOneUnit(unitMeasurement: number): number {
    const oneUnitPixelCount: string = (unitMeasurement / environment.UnitUpperLimit).toString();

    const oneUnit: number = oneUnitPixelCount.indexOf('.') !== -1
    // tslint:disable-next-line: radix
    ? parseInt(oneUnitPixelCount.substring(0, oneUnitPixelCount.indexOf('.') - 1))
    // tslint:disable-next-line: radix
  ...
  getPositionToPlace(cmdX: number, cmdY: number, unitMeasurement: number, cmdFacing: string): number[]
  {
 ...
 // tslint:disable-next-line: align
 getPositionForMove(pixX: number, pixY: number, newFacing: string, unitMeasurement: number): number[]
 {
    const posCordinates: number[] = [0, 0];
    this.oneUnitInPixels = this.calculatePixelForOneUnit(unitMeasurement);
  ...
  borderLimitCheck(newFacing: string): void {
    ...
    {
     this.pixY = this.pixY >= this.borderUpperLimit ?
     this.borderUpperLimit : this.pixY;

     this.pixY = this.pixY < this.borderLowerLimit ?
     this.borderLowerLimit : this.pixY;

     this.pixX = this.pixX >= this.borderUpperLimit ?
     this.borderUpperLimit : this.pixX;

     this.pixX = this.pixX < this.borderLowerLimit ?
     this.borderLowerLimit : this.pixX;     
   ...
   this.borderLowerLimit  : this.pixX; 
     ...
     this.pixY = this.pixY < this.borderLowerLimit ?
     environment.ImageLength : this.pixY;
    ...
    this.pixX = this.pixX < this.borderLowerLimit ?
    environment.ImageLength : this.pixX;
    }
  }
...
getRotationDegreeAsPerLeftRight(currFacing: string, direction: string): number
{
    let newRotationDegree = 0;
